...
def day5(crateMover9001: bool = False):
    ...
    for i in range(len(stacks_raw[0])):
        stack[i+1] = list(reversed([row[i] for row in stacks_raw if row[i] != " "])) # Process the input to populate each list in the dictionary
...
if __name__ == "__main__":
    print(f"The top of the stacks is {day5(False)} with the CrateMover 9000 and {day5(True)} with the CrateMover 9001")
